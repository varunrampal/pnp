...
export default async function handler(request, response) {
  ...
  try {
    ...
    const plantItems = Array.isArray(body.plantItems) ? body.plantItems.slice(0, 100).map(item => ({ plant: clean(item?.plant, 200), quantity: clean(item?.quantity, 60), size: clean(item?.size, 100) })).filter(item => item.plant || item.quantity || item.size) : [];
    ...
    const details = [['Name', fullName], ['Company', companyName || 'Not provided'], ['Email', email], ['Phone', clean(body.phone, 60) || 'Not provided'], ['Project location', clean(body.location, 160) || 'Not provided'], ['Required timing', clean(body.requiredBy, 80) || 'Not provided'], ['Project notes', projectNotes || 'Not provided']];
    ...
    const resendResponse = await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from: `${fromName} <${fromEmail}>`, to: [recipientEmail], reply_to: email, subject: `New quote request — ${companyName || fullName}`, html: `<div style="font-family:Arial,sans-serif;line-height:1.6;color:#26352d"><h2>New PEELS website quote request</h2><table style="border-collapse:collapse">${rows}</table><h3>Plant list</h3>${plantTable}${emailSignature}</div>`, ...(attachment ? { attachments: [attachment] } : {}) }) });
    ...
    try {
      const customerSummary = [['Project location', clean(body.location, 160) || 'Not provided'], ['Required timing', clean(body.requiredBy, 80) || 'Not provided'], ['Project notes', projectNotes || 'Not provided']];
